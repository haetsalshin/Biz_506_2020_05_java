 ...
 *  매개변수가 없는 기본 생성자가 매개변수를 필드변수 리스트를 매개변수로 받아서 
 *  필드변수 리스트를 매개변수로 받아서 필드변수 값을 지정하는 필드 생성자를 만들 수 있다.
 *  
 *  
 *  
 *  
...
public class AddrVO {
	...
	/*
	 *  필드변수가 String를 포함한 primitive(기본형) 변수형 일 때는
	 *  	초기화가 자동으로 이루어진다.
	 *  	문자열은 ""으로 숫자는 0으로
	 *  
	 *  Primitive(기본형) 변수형
	...
	// 객체생성 될 때 필드변수들에게 특별히 값을 만들어주고 싶을 때
	// 임의로 기본생성자를 만들고
	// 필드 변수들에 값을 Setting 할 수 있다.
	public AddrVO() {
	...
	// 필드(변수) 생성자
	/*
	 * 객체를 초기화 하면서 동시에 데이터를 Setting하고자 할 때 호출하는 생성자
	 */
	 ...
	 *  이 때 호출하여 사용하는 생성자
	 */
	// 나는 주소록 만드는데 name과 tel 만 입력하고 나머지는 나중에 입력하게 만들고 싶다.
