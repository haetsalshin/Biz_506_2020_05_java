...
public class AddrService01 {
	 ...
	 *  MullpointerException이 발생하면서 코드가 진행이 되지 않는다. => 초기화 시켜줘야 한다.
	 *  
	 *  아래의 1부터 3까지의 방법으로 반드시 변수를 초기화 해주어야 한다.
	 *  1. 필드변수를 선언과 동시에 초기화 하는 방법
	 *  	-> 메모리 관리상 여러가지 문제를 일으킬 수 있기 때문에 가급적 사용하지 말자
	 *  2. 생성자를 이용하여 초기화 하기.
	 *  3. 사용하기 직전에 초기화 하기(권장하는 방법은 아니다)
	 *  
	 *  객체형태의 변수란 클래스를 사용하여 선언 된 변수들
	...
	public void makeAddr() {
		...
		addrVO = new AddrVO(); // 이전에 만들어진 객체(인스턴스)를 재활용 하기 위해서는 반드시 다시 생성한다.
		addrVO.setName("이몽룡");
		...
		// 많은 데이터를 입력해야 할 때는 필드 생성자로 입력하는 것이 더 빠르다.
		addrVO = new AddrVO("성춘향", "010-333-3333", "남원시", 30, "이몽룡연인");
		addrList.add(addrVO);
