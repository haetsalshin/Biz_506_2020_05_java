...
public class String01 {
	...
	public static void main(String[] args) {
		/*
		 * 문자열형 변수는 변수가 생성될 때 실제 문자열이 담기는 변수 영역과 
		 * 문자열이 담긴 변수 영역의 주소를 갖고 있는 변수 영역 이렇게 2개가 생성이 된다.
		 * 한 곳은 실제 값이 담기고 한 곳은 주소가 담긴다.
		 * 
		 * 문자열을 == 비교연산자로 비교를 하면 
		 * 원칙은 주소로 비교를 하나, 어떨 때는 문자열 값으로 비교를 하기도 하고,
		 * 어떨 때는 주소로 비교를 한다.
		 * 
		 * 똑같은 문자열 임에도 불구하고 결과는 true, false 예측이 어렵다.
		 *  
		 * 따라서 문자열은 절대로 == 비교 연산자롸 비교하면 안된다.
		 * java에서는 !!!!
		 */
		String str1 = "Korea";
		String str2 = "Korea";
		 ...
		 *  
		 *  equals() : 문자열 형 변수의 저장된 실제 문자열이 같은가를 비교하는 method이다.
		 *  문자열이 같으면 true, 그렇지 않으면 false return하도록 정의 되어 있다.
		 */
		...
		System.out.println("Korea".equalsIgnoreCase("KOreA")); // true
		System.out.println("Korea".equalsIgnoreCase("KOREA1")); // false
		
		// 보통 아이디는 equals를 통해서 ,
		// 비밀번호는 equalsIgnoreCase를 통해서.
		// (왜냐면 요즘 대소문자 구분해서 비밀번호 치는 곳 많으니까)
