...
public class PrimeEx {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < nums.length ; i ++) {
			...
			// j 반복문이 끝났을 때 j값을 비교하는 용도로 사용하기 위해
			// for문 시작전에 변수를 선언했다.
			int j =0;
			for( j = 2; j < nums[i]; j ++) {
				/*
				...
				// j반복문 end
				// j반복문이 모두 반복되고 여기 도착했으면 nums[i]에 저장된 값을 소수가 된다.
				// 아니면 중간 break문을 만나서 여기 도착했으면 nums[i]에 저장된 값은 소수가 된다.
				// j반복문은 모두 반복되면 for반복문의 j < nums[i] 조건에 위배되어 반복문이 종료되고
				// j값은 == nums[i]가 된다.
				// 이러한 조건문은 코딩 상 썩 좋은 코드가 아니다
